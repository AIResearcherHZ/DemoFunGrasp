...
def train(cfg: TrainPipelineConfig):
    ...
    if hasattr(cfg.policy, "drop_n_last_frames"):
        ...
        shuffle = True
        sampler = None
    print(colored(f"accelerator.mixed_precision: {accelerator.mixed_precision}", "yellow", attrs=["bold"]))
    ...
    # num_epochs = cfg.steps // len(dataloader) + 1
    num_epochs = cfg.epochs
    for local_epoch_idx in range(1, num_epochs+1):
        with tqdm.tqdm(dataloader,
                    desc = colored(f"Training epoch {step // len(dataloader) + 1}", "green", attrs=["bold"]),
                    leave = False,
                    mininterval = 0.1,
                    colour='green') as tepoch:
            for batch_idx, batch in enumerate(tepoch):
                ...
                # according to cfg.log_freq, cfg.save_freq, cfg.eval_freq
                # is_log_step = cfg.log_freq > 0 and step % cfg.log_freq == 0
                # is_saving_step = step % cfg.save_freq == 0 or step == cfg.steps
                # is_eval_step = cfg.eval_freq > 0 and step % cfg.eval_freq == 0

                
                tepoch.set_postfix(loss=train_tracker.loss, refresh=False)
            # according to cfg.epochs, cfg.eval_epoch, cfg.log_epoch, cfg.save_epoch
            is_log_step = cfg.log_epoch > 0 and local_epoch_idx % cfg.log_epoch == 0
            is_saving_step = local_epoch_idx % cfg.save_epoch == 0 or local_epoch_idx == num_epochs
            is_eval_step = cfg.eval_epoch > 0 and local_epoch_idx % cfg.eval_epoch == 0
            if is_log_step:
                logging.info(colored(str(train_tracker), "yellow", attrs=["blink"]))
                if wandb_logger:
