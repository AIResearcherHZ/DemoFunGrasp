...
def load_object_point_clouds(object_files, asset_root,if_aff=False):
    ret = []
    ...
    for fn in object_files:
        ...
        if if_aff and pc.shape[1]==6:
            # add one to the end of the pcl
            pc = np.concatenate([pc,np.ones(pc.shape[0])],axis=-1)
        if sum(pc[:,-1]==0)==pc.shape[0]: # all zero
            pc[:,-1] = 1 
        ret.append(pc)
    return np.array(ret)
...
def rotate_vec_by_quat(q, v):
    # q: (...,4) (x,y,z,w), v: (...,3)
    q_con = quat_conjugate(q)
    v_as_quat = torch.cat([v, torch.zeros_like(v[..., :1])], dim=-1)  # (x,y,z,0)
    return quat_mul(quat_mul(q, v_as_quat), q_con)[..., :3]

def transform_points(quat, pt_input, trans=None):
    """
    quat: (4,) or (B,4) quaternion (x,y,z,w)
    pt_input: (N,6) or (B,N,6), [x,y,z,nx,ny,nz] in object frame
    trans: (3,) or (B,3) optional translation
    return: same shape, but in world frame
    """
    pos = pt_input[..., :3]
    norm = pt_input[..., 3:6]

    pos_world = rotate_vec_by_quat(quat, pos)
    norm_world = rotate_vec_by_quat(quat, norm)

    if trans is not None:
        pos_world = pos_world + trans

    return torch.cat([pos_world, norm_world], dim=-1)
...
if __name__=="__main__":
    # pc = get_pointcloud_from_mesh('../assets/union_object_dataset/meshes', 
    #                               '017_orange.stl',
    #                               num_sample=512)
    #print(pc)
    import time

    fns = os.listdir('../assets/temp/pointclouds')

    Q = queue.Queue(1)
    vis_pointcloud_realtime(Q, coord_len=0.1)
    for t in range(10000000):
        if t%100==0:
            pc = np.load(os.path.join('../assets/temp/pointclouds', fns[t//100]))
            print(fns[t//100], pc.shape, pc.dtype)
        Q.put(pc)
        time.sleep(0.01)